...
class CallBuilder {
  ...
  _appendUrlSegment(segment) {
    if ((0, _lodash.isNumber)(segment)) {
      ...
      if (segment.includes('/')) {
        ...
        let parsedLink = (0, _urijs2.default)(segment);
        let segments = parsedLink.path().split('/').filter(x => x.length > 0);
  ...
  _signRequest(config) {
    if (this._sdk.legacySignatures) {
      ...
      let digest = (0, _base.hash)(this._requestDigest(config));
  ...
  _signRequestLegacy(config) {
    let validUntil = Math.floor(this._getTimestamp() + SIGNATURE_VALID_SEC).toString();

    let fullUrl = this._getFullUrl(config);
    let signatureBase = `{ uri: '${fullUrl}', valid_untill: '${validUntil.toString()}'}`;
    let data = (0, _base.hash)(signatureBase);
  ...
  _getFullUrl(config) {
    let fullUrl = (0, _urijs2.default)(config.url);
    if (config.params) {
